    ...
    if (PIR4bits.RC3IF) {
        ...
        if (rxCharacter == '#') {
            ...
            if (rxCharacter == '$') {
                ...
                if (strncmp((char*)(decodedString+1),(char*)(lowbattery),(15)) == 0) {
                    iterator_field = fetchDigits(16);
                    clearDecodedString();
                    clearFieldByte();
                    sprintf(fieldByte,"%d",iterator_field+1);
                    lowBattery = 1;
                } else if (strncmp((char*)(decodedString+1),(char*)(resetslave),(10)) == 0) {
        ...
        PORTHbits.RH3 = 0;
        if (IOCEF2 == 1 && checkWaterFlow) {
            if (isPulseOn) {
               pulses++;
            }

        } else if (IOCEF3 == 1 && checkFertFlow) {
            if (isPulseOn) {
               pulses++;
            }
        ...
        if (IOCBF0 == 1) {
            IOCBF &= (IOCBF ^ 0xFF);
            if (PORTBbits.RB0 == 0) {
                _delay((unsigned long)((2500)*(64000000/4000.0)));_delay((unsigned long)((2500)*(64000000/4000.0)));
                if (PORTBbits.RB0 == 0) {
...
void __attribute__((picinterrupt(("low_priority")))) timerInterrupt_handler(void) {
    ...
    if (PIR0bits.TMR0IF) {
        ...
        if (sleepCount > 0 && PORTHbits.RH0 == 1) {
            sleepCount--;
            loraAliveCountCheck++;
            if (dryRunCheckCount == 0 || dryRunCheckCount < 3) {
                dryRunCheckCount++;
            }
            if (strncmp((char*)(decodedString+1),(char*)(alive),(5)) == 0 && strncmp((char*)(decodedString+6),(char*)(slave),(5)) == 0) {
                clearDecodedString();
                loraAliveCount++;
            }
            if (loraAliveCountCheck <= loraAliveCount+2) {
...
# 205 "main_1.c"
        ...
        else if (filtrationCycleSequence == 2 && Timer0Overflow == filtrationOnTime ) {
            Timer0Overflow = 0;
            PORTBbits.RB3 = 0;
            filtrationCycleSequence = 3;
        }
        else if (filtrationCycleSequence == 3 && Timer0Overflow == filtrationDelay2 ) {
            Timer0Overflow = 0;
            PORTBbits.RB4 = 1;
            filtrationCycleSequence = 4;
        }
        else if (filtrationCycleSequence == 4 && Timer0Overflow == filtrationOnTime ) {
            Timer0Overflow = 0;
            PORTBbits.RB4 = 0;
            filtrationCycleSequence = 5;
        }
        else if (filtrationCycleSequence == 5 && Timer0Overflow == filtrationDelay2 ) {
            Timer0Overflow = 0;
            PORTBbits.RB5 = 1;
            filtrationCycleSequence = 6;
        }
        else if (filtrationCycleSequence == 6 && Timer0Overflow == filtrationOnTime ) {
            Timer0Overflow = 0;
            PORTBbits.RB5 = 0;
...
nxtVlv: if (!valveDue && !phaseFailureDetected && !lowPhaseCurrentDetected) {
        ...
        if (valveDue && dueValveChecked) {
            ...
            dueValveChecked = 0;
            actionsOnDueValve(iterator_field);
