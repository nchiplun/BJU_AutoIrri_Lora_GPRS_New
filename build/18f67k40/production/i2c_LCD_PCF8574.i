...
# 2 "<built-in>" 2
# 1 "i2c_LCD_PCF8574.c" 2
# 12 "i2c_LCD_PCF8574.c"
# 1 "./congfigBits.h" 1
# 16 "./congfigBits.h"
...
# 16 "i2c_LCD_PCF8574.c" 2
# 153 "i2c_LCD_PCF8574.c"
...
void lcdInit(void) {
    ...
    lcd_i2cWriteByteSingleReg((0x4E), ((0b00000000 | 0b00000100) & ~0b00000001) & (~0b00000010));
    _delay((unsigned long)((100)*(64000000/4000000.0)));



    lcdWrite4Bits(0b00110000);
    _delay((unsigned long)((4500)*(64000000/4000000.0)));
...
void lcdWriteStringIndex(const char *message, size_t index) {
    unsigned char i = 0;
    while (i < index) {
        lcdDataWrite(*message++);
        i++;
...
void lcdSetCursor(unsigned char row, unsigned char col) {
  unsigned char row_offsets[4] = { 0x00, 0x40, 0x14, 0x54 };
  if ( row > _numlines ) {
    row = _numlines-1;
  }
  lcdCommandWrite(0x80 | ((col-1) + row_offsets[row-1]));
}
...
void lcdCreateChar(unsigned char location, unsigned char charmap_[]) {
    location &= 0x7;
    lcdCommandWrite((0x40 | ((location << 3) & 0xF8)));

    for (int i=0; i<8; i++)
        lcdDataWrite(charmap_[i]);
}
...
static void lcdSend(unsigned char value, unsigned char RsMode) {
    ...
    unsigned char lownib = (value << 4) & 0xFF;
    lownib &= 0xF0;
...
static void LCDwritePCF8574(unsigned char value) {
    lcd_i2cWriteByteSingleReg((0x4E), value | _lcdBacklightval);
}




void displayIcon(unsigned char icon) {
    lcdCreateChar(icon,charmap[icon]);
    lcdSetCursor(1,icon+1);
    lcdWriteChar(icon);
}

void removeIcon(unsigned char icon) {
    lcdCreateChar(icon, charmap[9]);
    lcdWriteChar(icon);
}
