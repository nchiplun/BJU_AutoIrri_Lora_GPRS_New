...
void lcdInit(void) {
    ...
    //create Char list
    lcdCreateChar(clock_icon, charmap[clock_icon]); // Clock icon

    lcdCreateChar(irri_icon, charmap[irri_icon]); // Irrigation icon

    lcdCreateChar(filt_icon, charmap[filt_icon]); // Filtration  icon

    lcdCreateChar(fert_icon, charmap[fert_icon]); // Fertigation icon

    lcdCreateChar(dry_icon, charmap[dry_icon]); // DryRun icon

    lcdCreateChar(phase_icon, charmap[phase_icon]); // Phase icon

    lcdCreateChar(battery_icon, charmap[battery_icon]); // Battery icon

    lcdCreateChar(sms_icon, charmap[sms_icon]); // SMS icon
...
void lcdCreateChar(unsigned char location, unsigned char charmap_[]) {
    ...
    lcdCommandWrite((LCD_CG_RAM_ADDRESS | ((location << 3) & 0xF8)));
...
void displayIcon(unsigned char icon) {
    lcdCreateChar(icon,charmap[icon]); // reload
    lcdSetCursor(1,icon+1);
    lcdWriteChar(icon);
}

void removeIcon(unsigned char icon) {
    lcdCreateChar(icon, charmap[blank_icon]); // Switch off icon
    lcdWriteChar(icon);  
}
/*
