...
#ifdef DEBUG_MODE_ON_H
    //********Debug log#start************//
    transmitStringToDebug("saveDeviceIDIntoEeprom_IN\r\n");
    //********Debug log#end**************//
#endif
    ////setBCDdigit(0x04,0); //  "4." BCD Indication for EEPROM Memory Write Operation
    for (iterator = 0; iterator < 12; iterator++) {
        __delay_ms(50);
        eepromWrite(eepromAddress[forDeviceID] + iterator, deviceId[iterator]);
    }
    __delay_ms(50);
    ////setBCDdigit(0x0F,0); // Blank "." BCD Indication for Normal Condition
#ifdef DEBUG_MODE_ON_H
    //********Debug log#start************//
    transmitStringToDebug("saveDeviceIDIntoEeprom_OUT\r\n");
    //********Debug log#end**************//
#endif
}
...
void saveFieldMappingIntoEeprom(void) {
    ...
    for (iterator = 0; iterator < 96; iterator++) {
        __delay_ms(50);
        eepromWrite(eepromAddress[forFieldMap] + iterator, fieldMap[iterator]);
    }
    __delay_ms(50);
    ////setBCDdigit(0x0F,0); // Blank "." BCD Indication for Normal Condition
#ifdef DEBUG_MODE_ON_H
...
void saveDeviceIdStatus(void) {
    ...
    ////setBCDdigit(0x04,0); //  "4." BCD Indication for EEPROM Memory Write Operation
    __delay_ms(50);
    eepromWrite(eepromAddress[forSystem] + 13, deviceIdSet);
    __delay_ms(50);
    ////setBCDdigit(0x0F,0); // Blank "." BCD Indication for Normal Condition
#ifdef DEBUG_MODE_ON_H
    //********Debug log#start************//
...
void loadDataFromEeprom(void) {
    ...
    for (iterator = 0; iterator < 12; iterator++) {
        __delay_ms(50);
        deviceId[iterator] = eepromRead(eepromAddress[forDeviceID] + iterator);
    }
#ifdef LCD_DISPLAY_ON_H
    ...
    factryPswrd[6] = '\0';
     for (iterator = 0; iterator < 96; iterator++) {
        __delay_ms(50);
        fieldMap[iterator] = eepromRead(eepromAddress[forFieldMap] + iterator);
    }
#ifdef LCD_DISPLAY_ON_H
    ...
    __delay_ms(50);
    deviceIdSet = eepromRead(eepromAddress[forSystem] + 13);
    __delay_ms(50);
