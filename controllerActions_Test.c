...
_Bool isMotorInNoLoad(void) {
    ...
    if (ctOutput > temp && ctOutput <= noLoadCutOff) {
        ...
        lcdCreateChar(5, charmap[dry_icon]); // dry run icon
        lcdSetCursor(1,19);
        lcdWriteChar(5);
#endif        
        #ifdef DEBUG_MODE_ON_H
...
#ifdef LCD_DISPLAY_ON_H
        lcdCreateChar(blank_icon, charmap[blank_icon]); // switch of dry run icon
        lcdSetCursor(1,19);
        lcdWriteChar(0);
#endif
        #ifdef DEBUG_MODE_ON_H
        ...
        for (field_No = 0; field_No < fieldCount; field_No++) {
            if (fieldValve[field_No].status == ON) {
                ...
                if (fieldValve[field_No].fertigationStage == injectPeriod) {
                    Fert_Motor = OFF; // Switch off fertigation valve in case it is ON
#ifdef LCD_DISPLAY_ON_H
                    lcdCreateChar(blank_icon, charmap[blank_icon]); // Switch off fertigation icon
                    lcdSetCursor(1,4);
                    lcdWriteChar(0);
#endif
					//Switch off all Injectors after completing fertigation on Period
...
#ifdef LCD_DISPLAY_ON_H
            lcdCreateChar(7, charmap[battery_icon]); // Battery icon
            lcdSetCursor(1,18);
            lcdWriteChar(7);
#endif
        __delay_ms(100);
...
#ifdef LCD_DISPLAY_ON_H
            lcdCreateChar(blank_icon, charmap[blank_icon]); // Switch off  icon
            lcdSetCursor(1,18);
            lcdWriteChar(0);
#endif
#ifdef DEBUG_MODE_ON_H
...
#ifdef LCD_DISPLAY_ON_H
        lcdCreateChar(6, charmap[phase_icon]); // Phase icon
        lcdSetCursor(1,17);
        lcdWriteChar(6);
#endif
...
void powerOnMotor(void) {
    ...
    Irri_Motor = ON;
#ifdef LCD_DISPLAY_ON_H
        lcdCreateChar(2, charmap[irri_icon]); // Irrigation icon
        lcdSetCursor(1,2);
        lcdWriteChar(2);
#endif
    Timer0Overflow = 0;  
...
#ifdef LCD_DISPLAY_ON_H
        lcdCreateChar(3, charmap[filt_icon]); // filtration icon
        lcdSetCursor(1,3);
        lcdWriteChar(3);
#endif
    } else {
...
void powerOffMotor(void) {
    ...
    __delay_ms(50);
#ifdef LCD_DISPLAY_ON_H
    lcdCreateChar(blank_icon, charmap[blank_icon]); // Switch off Irrigation icon
    lcdSetCursor(1,2);
    lcdWriteChar(0);
#endif
#ifdef LCD_DISPLAY_ON_H
    lcdCreateChar(blank_icon, charmap[blank_icon]); // Switch off Filtration icon
    lcdSetCursor(1,3);
    lcdWriteChar(0);
#endif
...
void setFactoryPincode(void) {
    ...
    if (DeviceBurnStatus == false) {
		...
		for (iterator = 0; iterator < 3; iterator++) { //Configure 3 Valves
			...
			fieldValve[iterator].field = iterator+1;
...
#ifdef LCD_DISPLAY_ON_H
                lcdCreateChar(4, charmap[fert_icon]); // Fertigation Icon
                lcdSetCursor(1,4);
                lcdWriteChar(4);
#endif
                // Injector code             
